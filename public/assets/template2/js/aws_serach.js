function search_withInput(input, type, id, table, status, divPage, load) {
    let data = {}
    if (type == "bank") {
...
function search_name(input, type, id, table, status, divPage, load) {
    let data = {
        data: input
    }
    ...
    function renderPage(page) {
        ...
        for (let index = startIndex; index < endIndex; index++) {
            ...
            link.onclick = function () {
                ...
                const allFigures = document.querySelectorAll('.post-thumb');
                allFigures.forEach(fig => fig.classList.remove('highlight'));

                // Add highlight to the clicked figure
...
async function renderDetail2(dataA, id) {
    ...
    if (from == "twitter") {
        if (dataA.hasOwnProperty('data')) {
            post_content = dataA.data.content;
        } else {
            post_content = dataA.text;
        }
